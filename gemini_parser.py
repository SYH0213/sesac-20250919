...
def parse_pdf_to_markdown(pdf_path: str, output_dir: str = "loaddata") -> str:
    ...
    try:
        ...
        print(f"[INFO] Gemini 2.5 Pro로 PDF 콘텐츠 생성 중...")
        response = client.models.generate_content(
            model="models/gemini-2.5-pro", contents=[file_to_parse, prompt]
        )
        ...
        if file_to_parse:
            ...
            client.files.delete(file=file_to_parse)

if __name__ == '__main__':
    pdf_file = "data/gemini-2.5-tech_1-3.pdf"
    try:
        md_path = parse_pdf_to_markdown(pdf_file)
        print(f"\n처리 완료. 마크다운 파일 경로: {md_path}")
    except Exception as e:
        print(f"\n오류 발생: {e}")